...
def fk_solve(angle1: float, angle2: float, angle3: float) -> Position3:
    x = np.cos(angle1) * (config.LEG_LEN1 + config.LEG_LEN3 * np.cos(angle2 + angle3) + config.LEG_LEN2 * np.cos(angle2))
    y = np.sin(angle1) * (config.LEG_LEN1 + config.LEG_LEN3 * np.cos(angle2 + angle3) + config.LEG_LEN2 * np.cos(angle2))
...
def limits_ok(thetas, margin=0.01):
    ...
    if not (config.MIN_JOINT2_RAD + margin <= theta2 <= config.MAX_JOINT2_RAD - margin):
        return False
    if not (config.MIN_JOINT3_RAD + margin <= theta3 <= config.MAX_JOINT3_RAD - margin):
        return False
    return True
...
def send_pulse(servo_id: int, angle: float, time_ms: int, dir_offset: int, board):
    pulse = 500 + angle / config.PI * 750 if dir_offset == 1 else 500 - angle / config.PI * 750
